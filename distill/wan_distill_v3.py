...
def parse_args():
    ...
    p.add_argument("--rank", type=int, default=32)  # v3: more capacity for the hard 1-step mapping
    p.add_argument("--lr", type=float, default=1e-4)
    p.add_argument("--steps", type=int, default=300)
    p.add_argument("--save-every", type=int, default=50)
    p.add_argument("--teacher-steps", type=int, default=4)
    p.add_argument("--student-steps", type=int, default=1)  # v3: target the 1-step frontier lightx2v can't do
    p.add_argument("--consistency", type=float, default=0.5,
                   help="weight on the self-consistency term (0 = pure MSE-to-teacher)")
    p.add_argument("--width", type=int, default=192)   # 256 thrashed swap w/ consistency's 2 graphs; 192 (~0.56x acts) fits
...
def one_step_from(model_patcher, positive, negative, x0_init, sigma_frac, seed):
    """Differentiable 1-step x0 prediction starting from a re-noised point.

    Re-noises x0_init to sigma = sigma_frac*sigma_max via the model's flow
    noise_scaling, then takes one euler step to 0 -> the student's x0 from a
    LIGHTER-noise start. Used for the self-consistency loss: a good 1-step
    student must predict the same clean frame from heavy and light noise.
    """
    ms = model_patcher.get_model_object("model_sampling")
    sigma_max = float(ms.sigma_max)
    sigma_start = sigma_max * float(sigma_frac)
    sigmas = torch.tensor([sigma_start, 0.0], device=x0_init.device, dtype=torch.float32)
    noise = comfy.sample.prepare_noise(x0_init, seed).to(x0_init.device)
    sampler = comfy.samplers.KSAMPLER(_diff_euler)
    guider = comfy.samplers.CFGGuider(model_patcher)
    guider.set_conds(positive, negative)
    guider.set_cfg(1.0)
    with torch.enable_grad():
        # latent_image=x0_init -> CFGGuider noise_scaling mixes it with noise at
        # sigma_start (max_denoise False since sigma_start<sigma_max).
        out = guider.sample(noise, x0_init, sampler, sigmas,
                            denoise_mask=None, disable_pbar=True, seed=seed)
    return out
    ...
    for step in range(1, args.steps + 1):
        ...
        with torch.no_grad():
            lat_T = sample_latent(model_t, pos_t, neg_t, latent_dict,
                                  ...
                                  with_grad=False)
        # --- Student: 1-step, grad on, LoRA on (differentiable) -------------
        ...
        lat_T = lat_T.to(lat_S.device)

        # Combined backward. The student and consistency forwards share upstream
        # saved tensors (conditioning/latent), so the two autograd graphs cannot
        # be freed independently -> they must be backwarded together. We capture
        # detached scalar values for logging/metadata so no graph is held past
        # opt.step(). Memory peak is controlled via resolution (see --width).
        # Teacher-anchor: student's 1-step jump should land on the teacher's x0.
        loss_boundary = F.mse_loss(lat_S, lat_T)
        cons_val = 0.0
        loss = loss_boundary
        # Self-consistency: student's 1-step prediction from a lighter-noise
        # re-noising of its own output should agree (consistency-model idea).
        if args.consistency > 0:
            try:
                lat_C = one_step_from(model_s, pos_t, neg_t,
                                      lat_S.detach(), 0.5, seed + 7)
                loss_cons = F.mse_loss(lat_C, lat_S.detach())
                loss = loss_boundary + args.consistency * loss_cons
                cons_val = float(loss_cons.detach())
            except Exception as exc:
                if step == 1:
                    print(f"[warn] consistency term disabled ({exc})", flush=True)

        opt.zero_grad(set_to_none=True)
        loss.backward()
        bound_val = float(loss_boundary.detach())
        torch.nn.utils.clip_grad_norm_(trainable, 1.0)
        opt.step()
        loss_val = bound_val + args.consistency * cons_val

        elapsed = time.time() - t0
        print(f"[step {step:04d}/{args.steps}] loss={loss_val:.5f} "
              f"(bound={bound_val:.4f} cons={cons_val:.4f})  "
              f"prompt={prompt[:40]!r}  elapsed={elapsed/60:.1f}m")
