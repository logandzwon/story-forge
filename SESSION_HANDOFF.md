...
2. **Mini Wan inference end-to-end NEVER validated:** Last session queued a test job (`mini_wan_job_id` in `/tmp/mini_wan_job.txt`) using mini_wan_render.py. Check status via `mini --done <jobid>`. If it succeeded, we have parallel rendering TODAY. If it failed, debug the ComfyUI workflow JSON in `mini_wan_render.py` against mini's actual node availability.
